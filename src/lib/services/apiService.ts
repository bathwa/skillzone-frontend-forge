...
class ApiService {
  ...
  async getEscrowAccounts(country?: Database['public']['Enums']['country_code']): Promise<ApiResponse<any[]>> {
    try {
      let query = supabase
      ...
      return {
        data: null,
        error: 'Failed to load escrow accounts',
        success: false,
      }
    }
  }

  async getSupportContacts(country?: Database['public']['Enums']['country_code']): Promise<ApiResponse<any[]>> {
    try {
      let query = supabase
      ...
      return {
        data: null,
        error: 'Failed to load support contacts',
        success: false,
      }
    }
  }
  ...
  async saveEscrowAccount(escrowData: {
    ...
    try {
      const { error } = await supabase
        .from('escrow_accounts')
        .insert([{
          country: escrowData.country,
          account_name: escrowData.account_name,
          account_number: escrowData.account_number,
          account_type: escrowData.account_type,
          provider: escrowData.provider,
          phone_number: escrowData.phone_number,
          is_active: true,
        }])
  ...
  async updateEscrowAccount(escrowId: string, updates: Partial<{
    account_name: string
    account_number: string
    account_type: 'mobile_wallet' | 'bank_account' | 'digital_wallet'
    provider?: string
  ...
  async updateSupportContact(contactId: string, updates: Partial<{
    phone: string
    email: string
    whatsapp: string
    is_active: boolean
    ...
    try {
      ...
      return {
        data: null,
        error: 'Failed to update support contact',
        success: false,
      }
    }
  }
